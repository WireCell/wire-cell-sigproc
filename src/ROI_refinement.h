...
namespace WireCell{
  namespace SigProc{
    ...
    public:
      ROI_refinement(Waveform::ChannelMaskMap& cmm,int nwire_u, int nwire_v, int nwire_w, float th_factor = 3.0, float fake_signal_low_th = 500, float fake_signal_high_th = 1000, float fake_signal_th_ind_factor=1.0, int pad = 5, int break_roi_loop = 2, float th_peak = 3.0, float sep_peak = 6.0, float low_peak_sep_threshold_pre = 1200, int max_npeaks = 200, float sigma = 2, float th_percent = 0.1); 
      ~ROI_refinement();
    ...
    private:
      ...
      float fake_signal_th_ind_factor;
      int pad;
      int break_roi_loop;
      float th_peak;
      float sep_peak;
